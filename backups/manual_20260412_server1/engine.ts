...
export class TradingEngine {
    ...
    try {
      ...
      for (const r of results) {
        ...
        if (r.passed >= minRequired) {
          ...
          this.log("info", `[SMART_BRAIN_DECISION] signal:${decision.signal} | decision:${decision.decision} | reason:${decision.summary}`);
          ...
          if (decision.decision === "EXECUTE" || decision.decision === "REDUCE") {
            ...
            if (validated) {
              if (decision.decision === "REDUCE") {
                ...
                try {
                  await this.openTrade(r.underlying, r.confirmations, validated, { signal: decision.signal, decision: decision.decision, reason: decision.summary });
                  break;
                } finally {
  ...
  private async validateOptionForEntry(underlying: string, conf: Confirmation[]): Promise<OptionQuote | null> {
    ...
    if (!opt.ticker || !opt.expiry || !(opt.strike > 0)) {
      ...
      this.log("warn", `[OPTION_REJECTED] ${underlying} ${ct}: contract data invalid`);
