 ...
 */
import type { Trade, BotConfig, BotStatus, BotLog, DailyStats, OverallStats, DailyHistory, Confirmation, Strategy, ContractType, TrailingConfig } from "../../shared/types.js";
...
import { saveTrade, closeTrade as dbCloseTrade, loadOpenTrades, loadAllTrades, saveLog, saveDailyStats, loadRecentLogs } from "./database.js";
import { newsFilter, type NewsFilterStatus } from "./news-filter.js";
import { notifyBotStart, notifyTradeEntry, notifyTradeExit, notifyTradeRejected, notifyError, notifyIBKRDisconnect, notifyStopLossHit, notifyNewsAlert, notifyDailyReport, notifyDecision } from "./notify.js";

type SmartBrainDecision = "EXECUTE" | "REDUCE" | "WAIT" | "ALERT_ONLY" | "REJECT";
...
export const DEFAULT_CONFIG: BotConfig = {
  ...
  capital: { mainCapital: 1500, paperBalance: 1500, carryDailyPnlIntoCapital: true },
  risk: {
    maxTradesPerDay: 5,
    maxOpenPositions: 1,
    maxDailyLossPercent: 60, maxConsecutiveLosses: 3, cooldownMinutes: 5
  },
  options: {
    deltaMin: 0.40, deltaMax: 0.60,
    minPremium: 0.5, maxPremium: 10,
    maxContracts: 10, contractsPerTrade: 1,
  ...
  zeroHero: {
    enabled: false, separateCapital: 200, maxTrades: 2,
...
export class TradingEngine {
    ...
    try {
      ...
      for (const r of results) {
        ...
        if (r.passed >= minRequired) {
          ...
          this.log("info", `[SMART_BRAIN] reason_codes: ${decision.reason_codes.join(", ") || "NONE"}`);
          this.log("info", `[SMART_BRAIN] summary: ${decision.summary}`);
          if (decision.emergency_stop) {
            this.log("warn", `[SMART_BRAIN] EMERGENCY_STOP active for ${r.underlying}`);
          }
          ...
          if (decision.decision === "EXECUTE" || decision.decision === "REDUCE") {
            ...
            this.log("info", `[ALERT_ONLY] Smart Brain طلب الإشعار فقط لـ ${r.underlying}`);
  ...
  private async evaluateWithSmartBrain(underlying: string, confirmations: Confirmation[], passed: number): Promise<SmartBrainResponse> {
    ...
    const requestPayload = {
      ...
      maxLoss: this.config.capital.mainCapital * (this.config.risk.maxDailyLossPercent / 100),
      emergencyStop: false,
      signal: {
        label: signal,
        underlying,
    ...
    const fallback = (reason: string): SmartBrainResponse => ({
      ...
      summary: `Fallback REJECT: ${reason}`,
    });
    ...
    try {
      ...
      return {
        ...
        decision: (data.decision as SmartBrainDecision) || "REJECT",
        ...
        reason_codes: Array.isArray(data.reason_codes) ? data.reason_codes : ["SMART_BRAIN_EMPTY_REASON_CODES"],
  ...
  private checkFilters(): string | null {
    ...
    const ml = this.config.capital.mainCapital * (this.config.risk.maxDailyLossPercent / 100);
  ...
  private async validateOptionForEntry(underlying: string, conf: Confirmation[]): Promise<OptionQuote | null> {
    ...
    try {
      ...
      try { notifyTradeRejected(underlying, "no contract"); } catch {} // notify
        this.log("warn", `[OPTION_REJECTED] ${underlying}: فشل Polygon API - ${e.message}`);
      return null;
    }

    if (!opt) {
      try { notifyTradeRejected(underlying, "no contract"); } catch {} // notify
        this.log("warn", `[OPTION_REJECTED] ${underlying} ${ct}: لا يوجد عقد بـ Delta ${this.config.options.deltaMin}-${this.config.options.deltaMax} | Premium $${this.config.options.minPremium}-$${this.config.options.maxPremium}`);
      return null;
    }

    if (opt.ask <= 0 || opt.ask <= opt.bid) {
      try { notifyTradeRejected(underlying, "no contract"); } catch {} // notify
        this.log("warn", `[OPTION_REJECTED] ${underlying} ${ct}: Bid/Ask غير منطقي (Bid:$${opt.bid} Ask:$${opt.ask})`);
      return null;
    }

    if (opt.volume < 100) {
      try { notifyTradeRejected(underlying, "no contract"); } catch {} // notify
        this.log("warn", `[OPTION_REJECTED] ${underlying} ${ct}: حجم ضعيف (${opt.volume} < 100 عقد) | ${opt.ticker}`);
      return null;
    }
    ...
    if (spreadPct > 15) {
      try { notifyTradeRejected(underlying, "no contract"); } catch {} // notify
        this.log("warn", `[OPTION_REJECTED] ${underlying} ${ct}: Spread عالي ($${spread.toFixed(2)} = ${spreadPct.toFixed(1)}% > 15%) | ${opt.ticker}`);
      return null;
    }
  ...
  private async openTrade(underlying: string, conf: Confirmation[], opt: OptionQuote) {
    const s = this.config.activeStrategy as Strategy;
    const trendConf = conf.find(c => c.name === "trend");
    const ct: ContractType = trendConf?.value?.includes("صاعد") ? "call" : "put";
    ...
    if (market.isIBKRConnected() && this.config.mode === "live") {
      ...
      const result = await ibkr.placeOrder(underlying, ct, opt.strike, opt.expiry, "BUY", this.config.options.contractsPerTrade, limitPrice);
    ...
    const t: Trade = {
      ...
      quantity: this.config.options.contractsPerTrade,
  ...
  private log(level: BotLog["level"], message: string, details?: Record<string, unknown>) {
    this.logs.unshift({ id: rid(), level, message, details, createdAt: Date.now() });
  ...
  getLogs(limit = 50): BotLog[] { return this.logs.slice(0, limit); }
  ...
  updateConfig(p: Partial<BotConfig>) {
    ...
    if (p.activeStrategy) this.config.activeStrategy = p.activeStrategy;
  ...
  getOverallStats(): OverallStats {
    ...
    return {
      ...
      currentCapital: Math.round((this.config.capital.mainCapital + tp) * 100) / 100
    };
  }
