...
console.log("[Market] Yahoo fallback disabled in trading path");
...
export class MarketDataProvider {
  ...
  async connectIBKR(): Promise<boolean> {
    ...
    this.ibkrMode = false;
    console.warn("[Market] IBKR connection failed. Trading path remains fail-safe: Polygon only for stock/options snapshots, no Yahoo fallback.");
    return false;
  }
  ...
  getDecisionContext(symbol: string, optionDataSource: "polygon" | "ibkr" | "none" = "none"): TradingDecisionContext {
    ...
    const polygonAvailable = POLYGON_ENABLED;
    let blockReason: string | undefined;
    if (!polygonAvailable) blockReason = "polygon_unavailable";
    if (!ibkrConnected) blockReason = blockReason || "ibkr_execution_unavailable";
    if (ibkrConnected && stockSource === "none") blockReason = `ibkr_stock_data_unavailable:${symbol}`;
    if (!ibkrConnected && stockSource === "none") blockReason = polygonAvailable ? `polygon_stock_data_unavailable:${symbol}` : "polygon_unavailable";
  ...
  async loadPrices(): Promise<boolean> {
    ...
    if (this.ibkrMode && ibkr.isConnected()) {
      console.log(`[Market] === IBKR MODE: All indicators from IBKR candles ===`);
      for (const t of tickers) {
        ...
        if (data) {
          ...
          console.error(`[DATA_BLOCK] ${t}: IBKR historical/live data unavailable | source=ibkr | polygonAvailable=${POLYGON_ENABLED} | yahooFallbackUsed=false`);
        }
        await delay(500);
      }
    } else if (POLYGON_ENABLED) {
    ...
    if (!this.loaded) {
      console.error("[Market] CRITICAL: Cannot load minimum required data");
    }
    return this.loaded;
  }
  ...
  async refreshPrices(): Promise<void> {
    for (const t of ["SPY", "QQQ"]) {
      ...
      const data = this.ibkrMode && ibkr.isConnected()
        ...
        : await fetchPolygonStock(t);
      if (data) {
        this.prices[t] = data;
      } else {
        delete this.prices[t];
        console.error(`[DATA_BLOCK] ${t}: refresh failed | source=${this.ibkrMode && ibkr.isConnected() ? "ibkr" : "polygon"} | ibkrConnected=${this.isIBKRConnected()} | polygonAvailable=${POLYGON_ENABLED} | yahooFallbackUsed=false`);
      }
      await delay(500);
    }
  }
