...
export class TradingEngine {
  ...
  private brokerAccountSnapshot: TastytradeAccountSnapshot | null = null;
  ...
  private enforceFuturesRuntimeConfig(): void {
    ...
    this.config.futures.dataSource = "tastytrade";
    this.config.futures.executionBroker = "tastytrade";
  ...
  private refreshBrokerAccountSnapshot(): void {
    if (!isFuturesMode()) return;
    try {
      const snapshot = readTastytradeAccountSnapshot();
      ...
      if (firstLoad || prevBalance !== snapshot.netLiquidatingValue) {
        this.log("info", `[TASTY_BALANCE] account=${snapshot.accountNumber} nlv=$${snapshot.netLiquidatingValue.toFixed(2)} dailyLossLimit=$${snapshot.dailyLossLimitAmount.toFixed(2)} futuresApproved=${snapshot.futuresApproved} source=${snapshot.source}`);
      }
    } catch (e: any) {
      this.log("warn", `[TASTY_BALANCE] ${e.message}`);
    }
  }

  private startBrokerAccountPolling(): void {
    if (!isFuturesMode() || this.brokerAccountPollTimer) return;
    this.refreshBrokerAccountSnapshot();
    this.brokerAccountPollTimer = setInterval(() => this.refreshBrokerAccountSnapshot(), FUTURES_BALANCE_REFRESH_MS);
  }
  ...
  private applyPaperDecisionThresholds(decision: SmartBrainResponse): SmartBrainResponse {
    ...
    const confidence = decision.confidence_final ?? 0;
    let thresholdDecision: SmartBrainDecision = "WAIT";
    let thresholdCode = "PAPER_THRESHOLD_WAIT_LT_72";
    if (confidence >= PAPER_EXECUTE_THRESHOLD) {
      thresholdDecision = "EXECUTE";
      thresholdCode = "PAPER_THRESHOLD_EXECUTE_GTE_75";
    } else if (confidence >= PAPER_REDUCE_THRESHOLD) {
      thresholdDecision = "REDUCE";
      thresholdCode = "PAPER_THRESHOLD_REDUCE_72_74";
    }

    const reasonCodes = Array.from(new Set([...(decision.reason_codes || []), thresholdCode]));
    const thresholdApplied = thresholdDecision !== decision.decision;
    return {
      ...decision,
      decision: thresholdDecision,
      reason_codes: thresholdApplied ? [...reasonCodes, "PAPER_DECISION_THRESHOLD_APPLIED"] : reasonCodes,
      summary: thresholdApplied ? `${decision.summary} | Paper threshold => ${thresholdDecision}` : decision.summary,
    };
  }
  ...
  async start() {
    ...
    if (ibkrConnected) {
      ...
      this.log("warn", "⚠️ IBKR غير متصل - لا يوجد Yahoo fallback، وسيبقى التداول محظورًا إذا فشلت بيانات IBKR/Polygon");
  ...
  private async parallelScan() {
    if (!this.running) return;
    if (isFuturesMode()) {
      this.logTradeModeGuard("options scan");
      return;
    }
    ...
    try {
      ...
      for (const r of results) {
        ...
        if (r.passed >= minRequired) {
          ...
          const finalSize = decision.decision === "REDUCE" ? Math.min(1, requestedSize) : (decision.decision === "EXECUTE" ? requestedSize : 0);
